...
import {
  ChevronLeft, Coffee, Car, Utensils, ShoppingBag, Edit3, ArrowRight,
...
const CURRENT_SPENT = 485.50;
const REMAINING     = TOTAL_LIMIT - CURRENT_SPENT; // R$ 114,50
...
export default function AddTransactionFlow() {
  ...
  const [payer,          setPayer]          = useState<"m1" | "m2" | null>(null);
  ...
  const handleFinalSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => { setIsSubmitting(false); router.push("/"); }, 600);
  };
  ...
  return (
    // h-screen + overflow-hidden garante que NADA fique fora do viewport
    <main className="flex flex-col h-screen overflow-hidden bg-[#18181b]">
          ...
          {[1, 2, 3].map((s) => (
            <div
              ...
              className={cn(
                ...
                s === step ? "w-5 h-2 bg-orange-500" : s < step ? "w-2 h-2 bg-orange-500/40" : "w-2 h-2 bg-white/10"
              )}
            />
          ))}
        </div>
      ...
      {/* ── Área de conteúdo: flex-1 + overflow-hidden → cada step preenche exatamente o espaço restante ── */}
      <div className="flex-1 overflow-hidden relative">

        {/* ── STEP 1 — Quem pagou? ── */}
        <div className={cn(
          "absolute inset-0 flex flex-col px-6 pt-7 pb-6 transition-all duration-350",
          ...
          <div className="flex flex-col gap-4">
            {[
              { id: "m1" as const, name: "Matheus", initial: "M", color: "bg-orange-500", activeRing: "border-orange-500/50" },
              { id: "m2" as const, name: "Amor",    initial: "A", color: "bg-purple-500", activeRing: "border-purple-500/50" },
            ].map((person) => (
              <button
                key={person.id}
                onClick={() => handlePayerSelect(person.id)}
                className={cn(
                  "flex items-center justify-between p-5 rounded-3xl border transition-all duration-250 active:scale-[0.98]",
                  payer === person.id ? cn(glassCardActive, person.activeRing) : glassCard
                )}
              >
                ...
                <div className={cn(
                  ...
                  payer === person.id ? "border-orange-500 bg-orange-500" : "border-neutral-700"
                )}>
            ...
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex flex-col items-center justify-center gap-3 py-5 rounded-3xl border transition-all duration-250 active:scale-[0.98]",
                    isSelected ? glassCardActive : glassCard
                  )}
                >
                  ...
                  <span className={cn("font-medium text-sm", isSelected ? "text-white" : "text-neutral-400")}>
                    {cat.label}
                  </span>
                </button>
              );
          ...
          {categoryId === "other" && (
            <form onSubmit={handleCustomCategorySubmit} className="mt-4 flex gap-2">
              <input
                ...
                className="flex-1 bg-white/[0.04] border border-white/10 rounded-2xl px-4 py-3 text-white placeholder:text-neutral-600 outline-none focus:border-orange-500/40 transition-colors text-sm"
              />
              <button
                type="submit"
                disabled={!customCategory.trim()}
                className="bg-orange-500 text-white p-3 rounded-2xl disabled:opacity-40 active:scale-95"
              >
                <ArrowRight size={20} />
              </button>
            </form>
        ...
        <div className={cn(
          ...
          <p className="text-neutral-500 text-sm mb-5">
            Disponível: <span className="text-white font-semibold">R$ {REMAINING.toFixed(2).replace(".", ",")}</span>
          </p>

          <form onSubmit={handleFinalSubmit} className="flex flex-col flex-1 gap-4 min-h-0">
            {/* Input de valor */}
            <div className={cn(
              "flex flex-col items-center justify-center py-5 rounded-3xl shrink-0",
              glassCard,
              wouldExceed && !exceedEnabled && "border-red-500/30"
            )}>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (dotIdx !== -1 && val.length - dotIdx > 3) return;
                  setAmount(val);
                }}
                className="w-full bg-transparent text-center text-[58px] font-extrabold tracking-tighter text-white outline-none placeholder:text-neutral-800 [-moz-appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              />

              {wouldExceed && (
                <div className={cn(
                  "flex items-center gap-2 mt-2 text-xs font-semibold px-3 py-1.5 rounded-full",
                  exceedEnabled ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"
                )}>
                  <AlertTriangle size={12} />
                  {exceedEnabled ? "Teto excedido — justificativa ativa" : `Excede o saldo (R$ ${REMAINING.toFixed(2).replace(".", ",")})`}
                </div>
              )}
              ...
              {exceedsWeekly && !wouldExceed && (
                <div className="flex items-center gap-2 mt-2 text-xs font-semibold px-3 py-1.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20">
                  <AlertTriangle size={12} />
                  Acima do limite semanal (R$ {WEEKLY_LIMIT.toFixed(0)})
                </div>
              )}
            </div>

            {/* Painel de excedente — scroll interno apenas nesta área se precisar */}
            {wouldExceed && (
              <div className={cn("flex flex-col gap-3 p-4 rounded-3xl border shrink-0", glassCard)}>
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => setExceedEnabled((v) => !v)}
                    className={cn("relative w-11 h-6 rounded-full transition-colors duration-300", exceedEnabled ? "bg-orange-500" : "bg-white/10")}
                  >
                ...
                {exceedEnabled && (
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {EXCEED_REASONS.map((reason) => (
                        <button
                          key={reason}
                          type="button"
                          onClick={() => { setExceedReason(reason); setCustomReason(""); }}
                          className={cn(
                            "flex items-center gap-2 px-3 py-2.5 rounded-xl border text-xs transition-all text-left",
                            exceedReason === reason
                    ...
                    <input
                      ...
                      value={customReason}
                      onChange={(e) => { setCustomReason(e.target.value); setExceedReason(""); }}
                      className="bg-white/[0.04] border border-white/10 rounded-xl px-3 py-2.5 text-white text-xs placeholder:text-neutral-600 outline-none focus:border-orange-500/40 transition-colors"
                    />
                  </div>
                )}
              </div>
            ...
            {/* Botão confirmar — empurrado para baixo */}
            <div className="mt-auto shrink-0">
              <button
                type="submit"
                disabled={!canSubmit || isSubmitting}
                className="relative w-full h-[56px] rounded-[18px] overflow-hidden text-white font-bold text-[15px] active:scale-[0.97] transition-all duration-200 flex items-center justify-center disabled:opacity-40 disabled:active:scale-100"
                style={{
                  background: canSubmit
                    ? "linear-gradient(135deg, #f97316 0%, #ea580c 50%, #c2410c 100%)"
                    : "rgba(255,255,255,0.06)",
                  boxShadow: canSubmit
                    ? "0 4px 0px #7c2d12, 0 8px 24px rgba(249,115,22,0.35), inset 0 1px 0 rgba(255,255,255,0.15)"
                    : "none",
                }}
              >
